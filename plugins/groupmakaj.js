...
ezra({ nomCom: "tagall1", categorie: 'Group', reaction: "🦠" }, async (dest, zk, commandeOptions) => {
      ...
      if (msgRepondu) {
        if (zkad) {
          if (membre) {
            ...
            } else { return repondre("This member is already an administrator of this group🦠.") }

          } else { return repondre("This user is not part of the group he might have fucked up."); }
        }
        else { return repondre("Sorry 😔, I cannot perform this action because I am not an administrator of the group.") }

      } else { repondre("please tag the member to be nominated to admin"); }
...
ezra({ nomCom: "demote", categorie: 'Group', reaction: "😬" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    if (autAdmin || superUser) {
      ...
      } else { repondre("please tag the member to be removed from this group"); }
    } else { return repondre("Sorry 😔 I cannot perform this action because you are not an administrator of the group.") }
  } catch (e) { repondre("oups " + e) }
...
ezra({ nomCom: "info", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
 ...
 ezra({ nomCom: "antilink", categorie: 'Group', reaction: "🚫" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      if(arg[0] === 'on') {
       ...
       if(enetatoui ) { repondre("the antilink is already activated for this group")
              ...
              repondre("Virusi Md antilink is activated successfully tell them to dare send a link") }
              ...
              if (enetatoui) { 
                ...
                repondre("Virusi Md antilink is not activated for this group");
              }
  ...
  } else { repondre('You are not entitled to this order calm down😁') ;
  }
